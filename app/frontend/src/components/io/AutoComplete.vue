<template>
  <div >
    <div class="input">
      <input @click="isFocused=true" @blur="delayedUnfocus" ref="inputField" type="text" v-model="filter" :placeholder="label" :required="required" autocomplete="off"
             @keydown.down="selected++" @keydown.up="selected--" @keydown.enter.prevent="select">
      <label>{{ label }} </label>
      <div v-if="filter.length>0" class="position-absolute m-2 cursor-pointer" style="right: 0" @click="filter=''; lockedIn=null">
        <unicon name="times-circle" icon-style="solid" :fill="colors.warning"/>
      </div>
    </div>
    <div  v-if="filter.length>0||isFocused" v-for="(option,i) in filteredOptions" class="b-1 b-rounded b-gray-25 position-relative m-b-1 cursor-pointer"
...
export default {
  ...
  setup({options, resultTitle, givenId}, {emit}) {
    const filter = ref('')
    ...
    const delayedUnfocus = () => setTimeout(()=>isFocused.value = false,100)
    ...
    setFilteredOptions();
    function setFilter(){
      if(typeof givenId !== 'undefined' && filteredOptions){
