...
export default function AddEvent({events, propagate, passedInEvent, clientId}){
    ...
    useEffect(()=>{
        ...
        if(passedInEvent){
            const initEvent = {...passedInEvent};
            if(typeof passedInEvent.start_date !== 'undefined'){
                initEvent.start_date = passedInEvent.start_date.replace(' ','T')
            }
            if(typeof passedInEvent.end_date !== 'undefined'){
                initEvent.end_date = passedInEvent.end_date.replace(' ','T')
            }
            console.log('fire 1')
            setEvent(initEvent)

            if(typeof passedInEvent.client_id !== 'undefined'){
        ...
        return function unsubscribe(){isSubscribed = false}
    },[passedInEvent])

    useEffect(()=>{
        const fetchData = async () => {
            if(clientId){
                const storeClient = await clientService.default.find(clientId);
                setBoundClient(storeClient[0]);
                setTimeout(()=>{
                    setEvent({...event, client_id: storeClient[0].id})
                },200)
    ...
    return (
        <form className={'grid'} onSubmit={store}>
            <div className="w-75p place-x-center">
                ...
                <div className="input">
                    <textarea placeholder={'Notes'} name={'notes'} value={event.notes || ''} onChange={updateEvent} rows={5}/>
                </div>
